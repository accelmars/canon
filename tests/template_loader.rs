...
use canon_core::template::{TemplateError, TemplateLoader, TemplateTier};
...
fn builtin_load() {
    ...
    let t = loader
        .load_by_name("test-builtin")
        .expect("built-in template should load");
...
fn workspace_load() {
    ...
    let t = loader
        .load_by_name("test-workspace")
        .expect("workspace template should load");
...
fn user_load() {
    ...
    let t = loader
        .load_by_name("test-user")
        .expect("user template should load");
...
fn explicit_path_override() {
    ...
    let t = loader
        .load_by_path(&path)
        .expect("explicit-path load should succeed");
    assert_eq!(t.manifest.name, "test-workspace");
    assert!(
...
fn missing_template_error_names_search_paths() {
    ...
    match err {
        TemplateError::NotFound { name, searched } => {
            ...
            assert!(
                !searched.is_empty(),
                "error should name at least one searched path"
            );
        }
